...
def test_simple_experiment(experiment_class, db_engine):
    with (
        ...
        with db_engine.connect() as conn:
            num_mgs = len([row for row in conn.execute(text("select * from triage_metadata.model_groups"))])
            ...
            # 6. experiment
            num_experiments = len([row for row in conn.execute(text("select * from triage_metadata.experiments"))])
