...
def _the_group(engine) -> int:
    with engine.connect() as conn:
        return conn.execute(
            text("select model_group_id from triage.model_groups limit 1")
        ).scalar_one()


def test_retrain_rejoins_original_group(db_engine_greenfield, tmp_path):
...
def test_retrain_and_predict_scores_the_fresh_model(db_engine_greenfield, tmp_path):
    engine = db_engine_greenfield
    _seed_source(engine)
    storage = str(tmp_path / "store")
    run_experiment(engine, _experiment_config(), storage_dir=storage, random_seed=42)
