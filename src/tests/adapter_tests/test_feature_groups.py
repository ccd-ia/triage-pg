...
# ------------------------------------------- named relationships (#9)

# The reporter's shape: two parallel named edges to one child (featurizer requires a name on
# a parallel edge, and the name replaces the child alias in every column it produces), plus a
# named forward transfer from a parent. Under the old parser every one of these columns fell
# through to the target group without a word.
_NAMED_CONFIG: dict[str, Any] = {
    "target": "games",
    "max_depth": 2,
    "intervals": ["P30D"],
    "aggregations": ["mean"],
    ...
    "entities": [
        {
            ...
            "id": "game_id",
            "table": "games",
            "variables": {"attendance": {"type": "numeric"}},
        },
        {
            "alias": "goalie_games",
            "id": "row_id",
            ...
            "variables": {"save_pct": {"type": "numeric"}},
        },
        {
            "alias": "teams",
            "id": "team_id",
            "table": "teams",
            "variables": {"elo": {"type": "numeric"}},
        },
    ],
    "relationships": [
        {
            "name": "home_goalies",
            "parent": {"entity": "games", "key": "game_id"},
            "child": {"entity": "goalie_games", "key": "home_game_id"},
        },
        {
            "name": "away_goalies",
            "parent": {"entity": "games", "key": "game_id"},
            "child": {"entity": "goalie_games", "key": "away_game_id"},
        },
        {
            "name": "home_team",
            "parent": {"entity": "teams", "key": "team_id"},
            "child": {"entity": "games", "key": "home_team_id"},
        },
    ],
}


def _partition_named(columns, config=_NAMED_CONFIG):
    return partition_features(
        columns,
        [e["alias"] for e in config["entities"]],
        target_alias=config["target"],
        relationships=config["relationships"],
    )


def test_named_relationship_columns_join_their_entity_not_the_target():
    # Names generated by featurizer itself, not written by hand, so a change in how it names
    # a named relationship fails here instead of silently regrouping.
    from triage.adapters.matrix import feature_labels

    columns = list(feature_labels(_NAMED_CONFIG))
    assert "MEAN(home_goalies.save_pct|interval=P30D)" in columns
    assert "home_team.elo" in columns

    assert _partition_named(columns) == {
        "games": ["attendance"],
        "goalie_games": [
            "MEAN(away_goalies.save_pct)",
            "MEAN(away_goalies.save_pct|interval=P30D)",
            "MEAN(home_goalies.save_pct)",
            "MEAN(home_goalies.save_pct|interval=P30D)",
        ],
        # a forward transfer reads the parent: the name stands for the far end
        "teams": ["home_team.elo"],
    }


def test_an_alias_inside_a_relationship_name_does_not_match_it():
    # 'own_goals.' contains 'goals.'; the old substring search sent the column to 'goals'.
    config = {
        "target": "games",
        "entities": [{"alias": "games"}, {"alias": "goals"}, {"alias": "shots"}],
        "relationships": [
            {
                "parent": {"entity": "games", "key": "game_id"},
                "child": {"entity": "goals", "key": "game_id"},
            },
            {
                "name": "own_goals",
                "parent": {"entity": "games", "key": "game_id"},
                "child": {"entity": "shots", "key": "game_id"},
            },
        ],
    }
    groups = _partition_named(["COUNT(goals.id)", "COUNT(own_goals.id)"], config=config)
    assert groups == {"goals": ["COUNT(goals.id)"], "shots": ["COUNT(own_goals.id)"]}


def test_the_outermost_named_relationship_wins_over_a_nested_alias():
    # MEAN(home_goalies.SUM(saves.x)): the outer stream is goalie_games; 'saves' is deeper.
    config = {
        **_NAMED_CONFIG,
        "entities": [*_NAMED_CONFIG["entities"], {"alias": "saves"}],
    }
    groups = _partition_named(["MEAN(home_goalies.SUM(saves.x))"], config=config)
    assert groups == {"goalie_games": ["MEAN(home_goalies.SUM(saves.x))"]}


def test_a_qualifier_nothing_declares_still_falls_back_but_says_so(monkeypatch):
    import triage.adapters.feature_groups as feature_groups_module

    warnings: list[str] = []
    monkeypatch.setattr(feature_groups_module.logger, "warning", warnings.append)
    groups = _partition_named(["attendance", "NEAREST(arenas_within_5km.capacity)"])
    assert groups == {"games": ["NEAREST(arenas_within_5km.capacity)", "attendance"]}
    assert len(warnings) == 1
    assert "['arenas_within_5km']" in warnings[0]
