...
from .utils import insert_model, insert_model_group
...
def filter_train_end_times(engine, train_end_times):
    with engine.begin() as conn:
        ...
        insert_model_group(conn, "modelType4", model_group_id=4)
        insert_model_group(conn, "modelType5", model_group_id=5)
        # model group 1
        insert_model(conn, 1, "2014-01-01")
        insert_model(conn, 1, "2015-01-01")
        insert_model(conn, 1, "2016-01-01")
        insert_model(conn, 1, "2017-01-01")
        # model group 2 only has one timestamp
        insert_model(conn, 2, "2014-01-01")
        # model group 3
        insert_model(conn, 3, "2014-01-01")
        insert_model(conn, 3, "2015-01-01")
        insert_model(conn, 3, "2016-01-01")
        insert_model(conn, 3, "2017-01-01")
        # model group 4 only has two timestamps
        insert_model(conn, 4, "2015-01-01")
        insert_model(conn, 4, "2016-01-01")
        # model group 5 only has three timestamps
        insert_model(conn, 5, "2014-01-01")
        insert_model(conn, 5, "2015-01-01")
        insert_model(conn, 5, "2016-01-01")
