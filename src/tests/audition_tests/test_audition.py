...
config = {
    ...
    "time_stamps": {
        "query": "SELECT DISTINCT train_end_time FROM triage.models "
...
def _seed_audition_data(db_engine, num_model_groups=10):
    """Seed model groups / models / test-split evaluations on the greenfield schema.

    Returns the list of model_group_ids (the generated identity ids) so callers
    ...
    with db_engine.connection() as conn:
        for model_type in model_types:
            ...
            for train_end_time in train_end_times:
                ...
                model_id = insert_model(conn, mgid, tet)
                for metric, parameter in metrics:
                    # one test-split evaluation per (model, metric); value is a
                    # plausible in-[0,1] number so thresholding has data to act on.
                    insert_evaluation(
                        conn,
                        model_id,
                        metric,
...
def test_Auditioner(db_pool_greenfield):
    ...
    auditioner = Auditioner(db_engine, model_group_ids, train_end_times, no_filtering)
