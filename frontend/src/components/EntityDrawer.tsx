 ...
 * outcome history, and (c) its score + rank trajectory across as_of_dates, defaulting
 * to the model group it was opened from with a toggle to overlay other model groups.
 * Data: GET /entities/{id}?experiment_hash= (migration 0006).
 */
      ...
      <aside className="sheet stacked" role="dialog" aria-label={`entity ${entityId}`}>
        <div className="sh">
          <div>
            ...
            <div className="sub mono">{experimentHash ? `${experimentHash.slice(0, 12)} · ` : ''}full profile</div>
          </div>
        ...
        {profile.loading ? (
          ...
          <div className="banner err">Failed to load entity: {profile.error.message}</div>
        ) : (
          <>
            <section>
              <h4>attributes</h4>
                  ...
                  <tbody>
                    {profile.data.label_history.map((l) => (
                      <tr key={`${l.as_of_date}-${l.label_timespan}`}>
                        ...
                        <td>
                          {l.outcome == null ? <span className="muted">—</span> : l.outcome > 0 ? (
                ...
                <div style={{ height: 200 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={chartData} margin={{ top: 6, right: 10, bottom: 0, left: -12 }}>
                      ...
                      <YAxis stroke="var(--mut)" tick={{ fontSize: 9 }} domain={field === 'rank_pct' ? [0, 1] : ['auto', 'auto']} />
                      <Tooltip contentStyle={{ background: 'var(--panel)', border: '1px solid var(--line)', fontSize: 11 }} />
                      {visibleGroups.length > 1 ? <Legend wrapperStyle={{ fontSize: 10 }} /> : null}
                      {visibleGroups.map((gid, i) => (
                        <Line
                          ...
                          dataKey={`g${gid}`}
                          name={labelFor(gid)}
                          stroke={gid === defaultGroupId ? 'var(--acc)' : LINE_COLORS[i % LINE_COLORS.length]}
                          strokeWidth={gid === defaultGroupId ? 2.5 : 1.5}
                          dot={false}
