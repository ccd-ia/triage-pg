...
- Status update (2026-07-18): pin bumped to **v0.9.1**. v0.9.x is **additive** — the text/graph feature families (φ-bridges) + the opt-in native 1-hop `graph_relationships` pass, with no DFS-planner change (v0.9.1 is "zero engine change"). The seam is unaffected: triage-pg forwards `feature_config` verbatim (only forcing `as_of_boundary: exclusive`), so the new families are reachable via passthrough — see [`docs/featurizer-passthrough-features.md`](../featurizer-passthrough-features.md). Scale verdict carries; not re-benchmarked (no engine change).
- Status update (2026-07-19): pin bumped to **v1.0.0** — the featurizer **stability release** (API freeze on the config schema + `Featurizer` surface, no new families, 0.9.x families scale-validated, Production/Stable). No triage-side change; full suite green (416 passed). Shipped with triage-pg **v1.0.0**.
- Status update (2026-08-07): pin bumped to **v1.0.1** — a bug-fix release triage-pg itself surfaced: an explicit empty `aggregations:`/`transformations:` list was silently swallowed (`config.get(...) or DEFAULT`) and fell back to the defaults; v1.0.1 honors it (`transformations: []` = no transform layer, byte-identical to the `[identity]` workaround). Feature naming unchanged (featurizer's snapshot test byte-identical); engine-version-in-identity (ADR-0016) makes the one-time feature-cache rebuild by design.
