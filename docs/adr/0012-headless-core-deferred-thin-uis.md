...
- Status update (2026-07-06): Headless parity restored for RESULT INSPECTION (v1-release plan P1/P6): `triage leaderboard` / `models` / `audition` / `model show` / `postmodel …` print the same views the dashboard reads (Rich tables, `--json` for scripting) — the CLI previously had zero result-inspection commands, which contradicted this ADR's spirit.
