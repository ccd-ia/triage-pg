...
create table triage.predictions (
    ...
    -- RESTRICT: append-only history (ADR-0006) — deleting a predicted model
    -- must fail loudly, never silently eat its predictions (ADR-0017).
...
10. **GC roots & retention (added 2026-06-12, ADR-0017).** GC collects **outputs, not history**: dead artifacts' outputs are deleted and rows flip to `status='collected'` (rebuild on demand from the pinned closure); row deletion is a separate explicit purge, bottom-up. Roots = artifacts **used** by runs of non-archived experiments (`run_artifacts` usage edges — `built_by_run` is not a liveness edge) plus predicted models; live = roots' upstream closure. `experiments.archived_at` is the root-removal gesture; `triage gc` is manual, dry-run by default. FK hardening lands with it: `predictions.model_id` and `artifact_inputs.parent_id` RESTRICT; `matrices.artifact_id`, `models.model_hash`, `cohorts.cohort_hash`, `labels.label_hash` CASCADE from `triage.artifacts`.

11. **The alembic stamp table belongs to the schema, not to `search_path` (added 2026-08-11).** `results_schema_versions` is pinned into `triage` (`version_table_schema`, `alembic/env.py`); the registry lineage pins `registry_schema_versions` into `registry`. Unqualified — how it stood through v1.1.1 — PostgreSQL resolved the name through the connecting role's `search_path` and created it in whatever schema was first there: invisible in triage-pg's own databases (`public`), a table in the *host project's* `raw` schema in the database where it surfaced. The migrations were never affected (they hardcode their schema); only the stamp floated. Two consequences fall out and are handled in code, not by hand: `env.py` creates the schema and relocates a pre-fix stamp **before** alembic's version-table bootstrap (which otherwise runs before the migration that creates the schema, and would replay `0001` against a populated database); and `0001`'s `downgrade` swaps the schema out from under its objects rather than cascading over the stamp, whose row alembic deletes after the migration returns. `drop schema triage cascade` therefore remains the whole teardown story for a project database, and no triage-pg object exists outside it.
